...
/*
    ...
    *-Slides & Intake-*
    ...
    Dpad Up - vSlide Higher Bucket
    Dpad Left - vSlide Specimen
    Dpad Right - vSlide grab from wall
    Left Bumper - vSlide Lower Bucket
    x - vSlides go Slightly Down
    b - vSlides go Slightly Up
...
@Config
@TeleOp(name="zSolo", group="Teleop")
public class teleOneMan extends OpMode {
    ...
    public void loop() {
        ...
        if (gamepad1.share && Button.BTN_LEVEL2.canPress(timestamp)){ // Lower Bucket
            robot.claw.setClose();
            clawOpen = false;
        ...
        // Wall
        if(gamepad1.dpad_right && Button.WALL.canPress(timestamp)) {
        ...
        if(gamepad1.x && Button.SLIGHT_UP.canPress(timestamp)){
        ...
        if(gamepad1.b && Button.SLIGHT_DOWN.canPress(timestamp)){
